...
namespace Prolab2_3Spo
{
    public partial class Form1 : Form
    {
        ...
        public bool addSarki(string sarkiAd, string muzisyen, string album, string sarkiUzunluk, string tur, string date)
        {
            getAlbumTur(album);
            if (String.Equals(albumTur, tur))
            {
                string query = $"INSERT INTO sarkilar (sarkiAd, sarkiSanatci, sarkiSure, sarkiDinlenmeSayi, sarkiTur, sarkiTarih) SELECT '{sarkiAd}','{muzisyen}', '{sarkiUzunluk}','0','{tur}','{date}' WHERE NOT EXISTS (SELECT sarkiID FROM sarkilar WHERE sarkiAd = '{sarkiAd}' AND sarkiSanatci ='{muzisyen}')";
        ...
        int albumID = 0;
        public bool searchalbumIDfromalbumler(string albumAd)
        {
        ...
        int sarkiIDsi;
        public void getsarkiID(string sarkiAd, string sarkiSanatci)
        {
            string query = $"SELECT sarkiID FROM sarkilar WHERE sarkiAd='{sarkiAd}' AND sarkiSanatci='{sarkiSanatci}';";
        ...
        private void buttonAppendSong_Click(object sender, EventArgs e)
        {
            ...
            else if (addSarki(songName, songSinger, songAlbum, songLong, songType, date))
            {
                MessageBox.Show($"{songName}-{songSinger} sarkilara basariyla eklendi!");
                searchalbumIDfromalbumler(songAlbum);

                add_to_albumsarki(albumID, searchsarkiIDfromsarkilar(songName, songSinger));
            }
            else
            {
        ...
        private void buttonDeleteAlbum_Click(object sender, EventArgs e)
        {
            ...
            if (searchAlbum(albumName, albumSinger))
            {
                searchalbumIDfromalbumler(albumName);
                deleteAlbumfromalbum_sanatci(albumID);
        ...
        private void buttonAppendAlbum_Click(object sender, EventArgs e)
        {
            ...
            else if (addAlbum(albumName, albumType, albumDate, albumSinger))
            {
                ...
                searchalbumIDfromalbumler(albumName);
                add_to_albumSanatci(albumID, search_sanatciID_fromSanatcilar(albumSinger));
            }
            else
            {
        ...
        private void buttonApplyUpdateSong_Click(object sender, EventArgs e)
        {
            ...
            String songOldAlbum = textAlbumNameAdd.Text;
            ...
            if (searchSarki(songOldName, songOldSinger))
            {
                if (updateSong(songOldName, songOldSinger, songNewName, songNewSinger, songNewAlbum, songNewLength, songNewDate, songNewTur))
                {
                    
                    if (searchalbumIDfromalbumler(songOldAlbum))
                    {
                        getsarkiID(songOldName,songOldSinger);
                        deleteSongfromalbum_sarki(sarkiIDsi);
                        add_to_albumsarki(albumID, sarkiIDsi);
                        MessageBox.Show($"{songOldName}-{songOldSinger} yeni bilgilerle basariyla güncellendi!");
                    }
                    else
                    {
                        MessageBox.Show($"{songOldAlbum} bulunamadi!");
                    }
        ...
        */
        ...
        private void buttonApplyUpdateSinger_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show($"{singerName} bulunamadi!");
            }
        ...
        private void buttonApplyUpdateAlbum_Click(object sender, EventArgs e)
        {
            ...
            if (searchAlbum(albumOldName, albumOldSinger))
            {              
                if (updateAlbum(albumOldName, albumOldSinger, albumNewName, albumNewSinger, albumNewDate, albumNewTur))
                {
                    if (searchalbumIDfromalbumler(albumOldName))
                    {
                        deleteAlbumfromalbum_sanatci(albumID);
                        search_sanatciID_fromSanatcilar(albumNewSinger);
                        add_to_albumSanatci(albumID, sanatciID);
                        MessageBox.Show($"{albumOldName}-{albumOldSinger} yeni bilgilerle basariyla güncellendi!");
                    }
                    else
                    {
                        MessageBox.Show($"{albumOldName} bulunamadi!");
                    }
            ...
            {
                MessageBox.Show($"{albumOldName}-{albumOldSinger} bulunamadi!");
            }
